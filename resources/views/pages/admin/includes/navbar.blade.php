<header class="w-full">
    <nav class="flex h-20 w-full items-center overflow-hidden rounded-lg bg-white shadow-sm">
        <div class="flex h-full w-60 items-center justify-center">
            <a href="{{ route('homepage') }}" class="h-[80%] bg-cover bg-center inline-block relative">
                <img src="{{ asset('assets/logo/logo_footer.png') }}" alt="logo" class="h-full">
            </a>
        </div>
        <div class="flex h-full flex-1 items-center justify-between px-5">
            ...
            <div class="flex items-center gap-3 rounded-md px-3">
                ...
                <a href="{{ route('logout') }}" class="font-bold transition-colors hover:text-red-500">Sign Out</a>
            </div>
        </div>
    </nav>
</header>
